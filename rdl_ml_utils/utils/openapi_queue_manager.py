...
class OpenAPIQueue:
    ...
    def _worker_loop(self) -> None:
        ...
        while not self._shutdown_event.is_set():
            ...
            client_lock = self._client_locks[task.client_idx]
            client_lock.acquire()
            try:
                # Execute the requested method
                task.result = task.method(*task.args, **task.kwargs)
    ...
    def _select_client(self) -> int:
        ...
        If all are busy, block until one becomes available.
        """
        while not self._shutdown_event.is_set():
            for idx, lock in enumerate(self._client_locks):
                if lock.acquire(blocking=False):
                    # Immediately release – the worker will re‑acquire.
                    lock.release()
                    return idx
    ...
    def _enqueue_task(
        ...
        # Resolve the method on the client (generate/chat)
        method = getattr(client, method_name)

        # Build the task using a dedicated helper
        task = self._create_task(client_idx, method, args, kwargs)

        # Queue the task and wait for completion via a reusable helper
    ...
    def _create_task(
        ...
        Centralising this logic makes it easier to adjust the task payload
