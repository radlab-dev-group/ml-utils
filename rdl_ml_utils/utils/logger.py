...
DEFAULT_LOGGER_FILE = "ml-utils.log"
...
def prepare_logger(
    ...
    l_file_name = (
        logger_file_name if logger_file_name is not None else DEFAULT_LOGGER_FILE
    )
    ...
    l_name = (
        ...
        else DEFAULT_LOGGER_FILE.replace(".log", "").replace("-", "_")
    )
